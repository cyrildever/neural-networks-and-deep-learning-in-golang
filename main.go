...
//
// `$ ./neuraldeep -n=2 -op=train -cost=crossEntropy -layers=784,300,10 -data=training -useMNIST=true -epochs=30 -size=10 -eta=3.0 -lambda=0.5 -eval=true -load=false`
...
func main() {
	...
	fmt.Printf("command to execute: $ ./neuraldeep -n=%s -op=%s -layers=%s -data=%s -label=%s -src=%s -useMNIST=%t -epochs=%d -size=%d -eta=%f -load=%t -eval=%t -cost=%s -lambda=%f\n===\n",
		*n, *operation, *layersStr, *dataStr, *labelStr, *src, *useMNIST, *epochs, *miniBatchSize, *eta, *load, *evaluate, *costFunction, *lambda)
	t0 := time.Now()
